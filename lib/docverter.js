	...
	_feed.on('change', function (change) {
		...
		fs.writeFile(tmpfile.path, change.doc.content, 'utf8', function (err) {
			if (err) {
				...
				form.append("from", formats[change.doc.format]);
				form.append("to", "html");
				// form.append("input_files[]", new Buffer(change.doc.content).toString('base64'));
				...
				form.submit({
					...
					if (err) {
						...
						console.log(
							"Change seq " + change.seq + " for doc id '" + change.id
							 + " --> Docverter '" + formats[change.doc.format] + "' to 'html'");
